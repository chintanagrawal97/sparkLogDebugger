...
import os
from flaskblog.models import Applog
# from flaskblog.models import Cluster
    ...
    for specificError in specificErrors:
        for key in specificError.keys():
            ...
            if pathdict.keys():
                ...
                print(errorPath) 
                print(description)
                applog = Applog(error=error,location = errorPath , description = description[0][0:1000] , cluster_app = cluster)
...
def LaunchDebugger(db,cluster):
    MainProgram(cluster.clusterId,cluster.applicationId,cluster.keyword,False,cluster.logPath)
    commitlogs(db,cluster)

    # print(cluster.id)
    # print(cluster.applicationId)
